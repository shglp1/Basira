...
async function assertSetupReady() {
  const supabase = createSupabaseServiceClient();
  const [{ data: version }, { count: itemCount }, { count: occCount }] = await Promise.all([
    supabase.from('assessment_versions').select('id').eq('id', ACTIVE_VERSION).maybeSingle(),
    supabase.from('items').select('*', { head: true, count: 'exact' }).eq('version_id', ACTIVE_VERSION),
    supabase.from('occupations').select('*', { head: true, count: 'exact' })
  ]);
  if (!version || !itemCount || !occCount) throw new Error('SETUP_REQUIRED');
}
